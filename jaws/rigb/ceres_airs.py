...
def main():
    indir = 'grl/'
    outdir = 'ceres-airx3std/grl/'

    stn_names, lat_lon_stn = nomiss_airs.get_stn_latlonname('stations_radiation_grl.txt')

    for infile in glob.iglob(indir+'ceres.*.nc'):
        print(infile)
        ...
        for _ in stn_new:
            ...
            outfile = outdir+'/'+stn_names[x_coord]+'.'+basename[-7:-3]+'.'+'ceres'+'.nc'
